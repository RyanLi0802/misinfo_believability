...
def extract_features(data):
    ...
    for idx in range(data.shape[0]):
        ...
        meta_data = np.concatenate((row[2:7].to_numpy(dtype=np.float64), row[8:10].to_numpy(dtype=np.float64)))
        ...
        # liwc
        liwc_columns = ['WC', 'WPS', 'they', 'number', 'adverb', 'Drives', 'power', 'cause', 'discrep', 'friend', 'politic', 'leisure', 'relig', 'need', 'fatigue', 'allure', 'visual', 'focuspast', 'Conversation', 'AllPunc', 'Comma', 'Exclam', 'OtherP']
        liwc = row[liwc_columns].to_numpy(dtype=np.float64)
        
        row_features = np.concatenate((meta_data, sentiment, emotions, topic, toxicity, liwc))
