...
def is_crh(scoredNotes, minRatingsNeeded, crhThreshold) -> pd.Series:
  return (scoredNotes[c.numNotesKey] >= minRatingsNeeded) & (
...
def compute_scored_notes(
  ...
  #   ).timestamp()
  # )
  notesToUse = pd.DataFrame(
    notes[[c.tweetIdKey, c.believableNumKey]]
  )
  notesToUse.loc[:, c.numNotesKey] = 1
  ...
  tweetStats = notesToUse.groupby(c.tweetIdKey).sum()

  noteParamsColsToKeep = [c.tweetIdKey, c.noteInterceptKey, c.noteFactor1Key]
  for col in c.noteParameterUncertaintyTSVColumns:
    if col in noteParams.columns:
      noteParamsColsToKeep.append(col)
  tweetStats = tweetStats.merge(noteParams[noteParamsColsToKeep], on=c.tweetIdKey)
  print(tweetStats.columns)
  print()
  print(notes.columns)  
  ...
  rules = [
    ...
    scoring_rules.RuleFromFunction(
      RuleID.GENERAL_CRH,
      {RuleID.INITIAL_NMR},
      c.currentlyRatedBelievable,
      lambda noteStats: is_crh_function(tweetStats, minRatingsNeeded, crhThreshold),
    ),
    scoring_rules.RuleFromFunction(
      RuleID.GENERAL_CRNH,
      {RuleID.INITIAL_NMR},
      c.currentlyRatedUnbelievable,
      lambda noteStats: is_crnh_function(
        tweetStats, minRatingsNeeded, crnhThresholdIntercept, crnhThresholdNoteFactorMultiplier
      ),
    ),
    scoring_rules.NMtoCRNH(RuleID.NM_CRNH, {RuleID.INITIAL_NMR}, c.currentlyRatedUnbelievable),
  ]
  ...
  #     ),
  #   )
  tweetStats[c.firstTagKey] = np.nan
  tweetStats[c.secondTagKey] = np.nan
  scoredTweets = scoring_rules.apply_scoring_rules(tweetStats, rules)
  # Discard the locked status column since it is captured in noteStatusHistory and
  # not necessary for the rest of scoring.
  # scoredTweets = scoredTweets.drop(columns=[c.lockedStatusKey])
