...
def preprocess_data(
  ...
  ratings: pd.DataFrame,
  noteStatusHistory: pd.DataFrame,
  shouldFilterNotMisleadingNotes: bool = False,
  logging: bool = True,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
  ...
  #     ratings (pd.DataFrame)
  #     noteStatusHistory (pd.DataFrame)
  #     shouldFilterNotMisleadingNotes (bool, optional): Defaults to False.
  ...
  notes.loc[:, c.believableNumKey] = np.nan
  notes.loc[notes[c.believableLevelKey] == c.notBelievableValueTsv, c.believableNumKey] = 0
  notes.loc[notes[c.believableLevelKey] == c.believableValueTsv, c.believableNumKey] = 1
  notes = notes.loc[~pd.isna(notes[c.believableNumKey])]
  ...
  if logging:
    print(
      "Num Notes: %d, Num Unique Tweets Rated: %d, Num Unique Noters: %d"
      % (
        len(notes),
        len(np.unique(notes[c.tweetIdKey])),
        len(np.unique(notes[c.noteAuthorParticipantIdKey])),
      )
    )
  return notes, ratings, noteStatusHistory


def filter_ratings(notes: pd.DataFrame, logging: bool = True) -> pd.DataFrame:
  ...
  n = notes.groupby(c.tweetIdKey).size().reset_index()
  tweetsWithMinNumNotes = n[n[0] >= c.minNumRatersPerNote]

  notesFiltered = notes.merge(tweetsWithMinNumNotes[[c.tweetIdKey]], on=c.tweetIdKey)

  if logging:
    print("Filter tweets with too few notes")
    print(
      "  After Filtering tweets w/less than %d notes, Num notes: %d, Num Unique tweets Rated: %d, Num Unique Raters: %d"
      % (
        c.minNumRatersPerNote,
        len(notesFiltered),
        len(np.unique(notesFiltered[c.tweetIdKey])),
        len(np.unique(notesFiltered[c.noteAuthorParticipantIdKey])),
      )
    )
