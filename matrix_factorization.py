...
def get_note_and_rater_id_maps(
  ...
  tweetData = notes[[c.tweetIdKey, c.noteAuthorParticipantIdKey, c.believableNumKey]]
  assert not pd.isna(tweetData).values.any(), "tweetData must not contain nan values"

  noterIdMap = (
    pd.DataFrame(tweetData[c.noteAuthorParticipantIdKey].unique())
    .reset_index()
    .set_index(0)
    .reset_index()
    .rename(columns={0: c.noteAuthorParticipantIdKey, "index": c.noterIndexKey})
  )

  tweetIdMap = (
    pd.DataFrame(tweetData[c.tweetIdKey].unique())
    .reset_index()
    .set_index(0)
    .reset_index()
    .rename(columns={0: c.tweetIdKey, "index": c.tweetIndexKey})
  )

  tweetNoterIds = tweetData.merge(tweetIdMap, on=c.tweetIdKey)
  tweetNoterIds = tweetNoterIds.merge(noterIdMap, on=c.noteAuthorParticipantIdKey)

  return tweetIdMap, noterIdMap, tweetNoterIds
...
def initialize_parameters(
  ...
  if noteInit is not None:
    ...
    noteInit = tweetIdMap.merge(noteInit, on=c.tweetIdKey, how="left")
  ...
  if userInit is not None:
    ...
    userInit = noterIdMap.merge(userInit, on=c.noteAuthorParticipantIdKey, how="left")
    mf_model.user_intercepts.weight.data = torch.tensor(
      np.expand_dims(userInit[c.noteAuthorParticipantIdKey].astype(np.float32).values, axis=1)
    )
...
def create_mf_model(
  ...
  Returns:
      Tuple[BiasedMatrixFactorization, torch.optim.Optimizer, torch.nn.modules.loss._Loss]
  """
  n_users = tweetNoterIds[c.noterIndexKey].nunique()
  n_items = tweetNoterIds[c.tweetIndexKey].nunique()
  if logging:
    print("------------------")
    print(f"Noters: {n_users}, Tweets: {n_items}")
  ...
  initialize_parameters(
    mf_model, tweetIdMap, noterIdMap, noteInit, userInit, globalInterceptInit, numFactors
  )
...
def run_mf(
  ...
  globalInterceptInit: Optional[float] = None,
  specificTweetId: Optional[int] = None,
  ...
  tweetIdMap, noterIdMap, tweetNoterIds = get_note_and_rater_id_maps(ratings)
  ...
  if specificTweetId is not None:
    # Only used for quick, approximate analysis to score a particular note
    mf_model.freeze_rater_and_global_parameters()
    tweetNoterIdsForSpecificNote = tweetNoterIds.loc[tweetNoterIds[c.tweetIdMap] == specificTweetId]
    rating = torch.FloatTensor(tweetNoterIdsForSpecificNote[c.believableNumKey].values).to(
      mf_model.device
    )
    row = torch.LongTensor(tweetNoterIdsForSpecificNote[c.noterIndexKey].values).to(mf_model.device)
    col = torch.LongTensor(tweetNoterIdsForSpecificNote[c.tweetIndexKey].values).to(mf_model.device)
  else:
    # Normal case
    rating = torch.FloatTensor(tweetNoterIds[c.believableNumKey].values).to(mf_model.device)
    row = torch.LongTensor(tweetNoterIds[c.noterIndexKey].values).to(mf_model.device)
    col = torch.LongTensor(tweetNoterIds[c.tweetIndexKey].values).to(mf_model.device)
  ...
  assert mf_model.item_factors.weight.data.cpu().numpy().shape[0] == tweetIdMap.shape[0]
  ...
  fitNoteParams, fitRaterParams = get_parameters_from_trained_model(
    mf_model, tweetIdMap, noterIdMap, flipFactorsForIdentification, numFactors
  )
