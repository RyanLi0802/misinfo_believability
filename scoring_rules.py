...
class DefaultRule(ScoringRule):
  ...
  def score_notes(
    ...
    noteStatusUpdates = pd.DataFrame(noteStats[[c.tweetIdKey]])
...
class NMtoCRNH(ScoringRule):
  ...
  def score_notes(
    ...
    noteStatusUpdates = noteStats.loc[(noteStats[c.noteInterceptKey] < c.crnhThresholdNMIntercept)][[c.tweetIdKey]]
    noteStatusUpdates[c.ratingStatusKey] = self._status
    return (noteStatusUpdates, None)
...
def apply_scoring_rules(tweetStats: pd.DataFrame, rules: List[ScoringRule]) -> pd.DataFrame:
  ...
  Args:
    tweetStats: attributes, aggregates and raw scoring signals for each tweet.
    rules: ScoringRules which will be applied in the order given.

  Returns:
    tweetStats with additional columns representing scoring results.
  """
  # Initialize empty dataframes to store labels for each note and which rules impacted
  # scoring for each note.
  
  tweetLabels = pd.DataFrame.from_dict({c.tweetIdKey: [], c.ratingStatusKey: []}).astype(
    {c.tweetIdKey: np.int64}
  )
  tweetRules = pd.DataFrame.from_dict({c.tweetIdKey: [], c.activeRulesKey: []}).astype(
    {c.tweetIdKey: np.int64}
  )
  tweetColumns = pd.DataFrame.from_dict({c.tweetIdKey: []}).astype({c.tweetIdKey: np.int64})
  ...
  for rule in rules:
    ...
    tweetStatusUpdates, additionalColumns = rule.score_notes(tweetStats, tweetLabels)
    if additionalColumns is not None:
      assert set(tweetStatusUpdates[c.tweetIdKey]) == set(additionalColumns[c.tweetIdKey])
    # Update noteLabels, which will always hold at most one label per note.
    tweetLabels = pd.concat([tweetLabels, tweetStatusUpdates]).groupby(c.tweetIdKey).tail(1)
    # Update note rules to have one row per rule which was active for a note
    tweetRules = pd.concat(
      [
        tweetRules,
        pd.DataFrame.from_dict(
          {c.tweetIdKey: tweetStatusUpdates[c.tweetIdKey], c.activeRulesKey: rule.get_name()}
        ),
      ]
    )
    # Merge any additional columns into current set of new columns
    if additionalColumns is not None:
      assert {c.tweetIdKey} == (set(tweetColumns.columns) & set(additionalColumns.columns))
      tweetColumns = tweetColumns.merge(additionalColumns, on=c.tweetIdKey, how="outer")
  ...
  tweetRules = tweetRules.groupby(c.tweetIdKey).aggregate(list).reset_index()
  tweetRules[c.activeRulesKey] = [
    ",".join(activeRules) for activeRules in tweetRules[c.activeRulesKey]
  ]
  # Validate that there are labels and assigned rules for each note
  assert set(tweetStats[c.tweetIdKey]) == set(tweetLabels[c.tweetIdKey])
  assert set(tweetStats[c.tweetIdKey]) == set(tweetLabels[c.tweetIdKey])
  assert len(set(tweetColumns[c.tweetIdKey]) - set(tweetStats[c.tweetIdKey])) == 0
  # Merge note labels, active rules and new columns into noteStats to form scoredNotes
  scoredNotes = tweetStats.merge(tweetLabels, on=c.tweetIdKey, how="inner")
  scoredNotes = scoredNotes.merge(tweetRules, on=c.tweetIdKey, how="inner")
  scoredNotes = scoredNotes.merge(tweetColumns, on=c.tweetIdKey, how="left")
  assert len(scoredNotes) == len(tweetStats)
  # Set boolean columns indicating scoring outcomes
  scoredNotes[c.currentlyRatedHelpfulBoolKey] = (
    scoredNotes[c.ratingStatusKey] == c.currentlyRatedBelievable
  )
  scoredNotes[c.currentlyRatedNotHelpfulBoolKey] = (
    scoredNotes[c.ratingStatusKey] == c.currentlyRatedUnbelievable
  )
